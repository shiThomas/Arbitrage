...
This function runs bellman-ford for V-1 iterations, then runs one extra 
iteration to find a negative cycle. Then it traces back to return that cycle.

INPUTS
currencies: An object of the currencies class which includes an adjacency List 
and an adjacency Matrix
tol: Tolerance and Machine Epsilon

OUTPUT
cycle: Returns an array with the ranks of vertexes in the negative cycle,
or return an empty array if there is no negative cycle.
...
def detectArbitrage(currencies, tol=1e-15):
    ...
    for it in range(0, len(currencies.adjList)-1):
        ...
        for u in currencies.adjList:
            for neigh in u.neigh:
                ...
                if neigh.dist > u.dist\
                        + currencies.adjMat[u.rank][neigh.rank] + tol:
                    neigh.dist = u.dist\
                                 + currencies.adjMat[u.rank][neigh.rank]
                    neigh.prev = u

    # run bellman-ford one more time to find negative cycle
    # flag turns to true if a negative cycle is detected
    flag = False
    cycle = []
    tmp = []
    start = None
    ...
    for u in currencies.adjList:
        for neigh in u.neigh:
            ...
            # If there is a negative cycle, update the previous vertex
            if neigh.dist > u.dist\
                    + currencies.adjMat[u.rank][neigh.rank] + tol:
                neigh.prev = u

                # Set flag to true and cycle start as the current vertex
                start = u
                flag = True

    # if there is a negative cycle, append each vertex's rank to the tmp first
    if flag is True:
        back = start
        tmp.append(start.rank)

        # Trace back until one element appears second time in the tmp
        while back.prev.rank not in tmp:
            back = back.prev
            tmp.append(back.rank)

        # set the new cycle start as the element which appears second time
        start = back.prev
        current = start

        # append the start point into cycle
        cycle.append(start.rank)

        # append each point until reach the start point
        while current.prev is not start:
            current = current.prev
            cycle.append(current.rank)

        # append the start point so that start is equal to end
        cycle.append(start.rank)
...
This function takes in the rates and returns the correct weighted edges 
to the adjacency Matrix.

INPUT
rates: The 2D matrix representing the exchange rates between currencies.

OUTPUT
The adjacency Matrix with the correct weighted edges.
"""
...
def rates2mat(rates):
    ...
    # For each rate between two currencies, call it R
    # calculate logarithm on 1/R and return the adjacency matrix
    return [[math.log10(1/R) for R in row] for row in rates]
    ##### Your implementation goes here. #####
...
# test the result of rates2mat
# Currencies(0).printMat()
# Currencies(0).printList()
